...
// ============================================================
// 13. REPORTS PAGE — KPI + Export Excel
// ============================================================
...
function ReportsPage({tickets}) {
  ...
  const filtered = tickets.filter(t => {
    const inPeriod  = (t.createdAt||0) >= getStartDate();
    const inMember  = !filtMember || t.assignee === filtMember;
    return inPeriod && inMember;
  });
  ...
  // ── KPI par statut (pour le graphique en barres)
  const statusStats = COLUMNS.map(col => ({
    col,
    count: filtered.filter(t=>t.status===col).length,
    cfg:   COL_CFG[col],
  }));
  ...
  const exportToExcel = () => {
    ...
    // Feuille 1 : Tous les tickets
    const ticketRows = [
      ["ID","Titre","Description","Statut","Priorité","Assigné à","Tags","Date création","Date assignation"],
      ...filtered.map(t => [
        `#${t.id}`,
        t.title,
        t.description||"",
        t.status,
        t.priority,
        getMember(t.assignee)?.name||"Non assigné",
        (t.tags||[]).join(", "),
        t.createdAt  ? new Date(t.createdAt).toLocaleDateString("fr-FR")  : "",
        t.assignedAt ? new Date(t.assignedAt).toLocaleDateString("fr-FR") : "",
      ])
    ];

    // Feuille 2 : KPI par membre
    const memberRows = [
      ["Membre","Total tickets","Terminés","En attente","Taux de clôture"],
      ...memberStats.map(s => [
        s.member.name,
        s.total,
        s.done,
    ...
    const globalRows = [
      ...
      ["Tickets critiques",critical],
      ["Période",          period==="week"?"7 derniers jours":period==="month"?"30 derniers jours":"Toutes les dates"],
      ["Exporté le",       new Date().toLocaleDateString("fr-FR")],
    ];

    // Conversion en CSV multi-sections
  ...
  const label = {fontSize:11,fontWeight:700,color:"#ADB5BD",letterSpacing:".5px",textTransform:"uppercase",marginBottom:4};
  const value = {fontSize:28,fontWeight:700,color:"#1a1a1a",lineHeight:1};
  ...
  return (
    <div style={{animation:"fadeIn .3s ease"}}>
        ...
        <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
          ...
          <div style={{display:"flex",background:"#fff",border:"1px solid #E8EAED",borderRadius:8,overflow:"hidden"}}>
            {[{id:"all",label:"Tout"},{id:"week",label:"7 jours"},{id:"month",label:"30 jours"}].map(p=>(
              <button key={p.id} onClick={()=>setPeriod(p.id)} style={{padding:"7px 14px",border:"none",background:period===p.id?"#1a1a1a":"transparent",color:period===p.id?"#fff":"#666",fontSize:12,fontWeight:600,cursor:"pointer",transition:"all .15s",fontFamily:"inherit"}}>
                {p.label}
              </button>
            ))}
          </div>
          ...
          <span style={{fontSize:12,color:"#ADB5BD"}}>{filtered.length} ticket{filtered.length>1?"s":""} dans la sélection</span>
        </div>

        {/* Bouton export */}
        <button onClick={exportToExcel} disabled={exporting}
          style={{display:"flex",alignItems:"center",gap:8,background:exporting?"#F8F9FA":"#1a1a1a",color:exporting?"#ADB5BD":"#fff",border:`1px solid ${exporting?"#E8EAED":"#1a1a1a"}`,borderRadius:8,padding:"8px 18px",fontSize:13,fontWeight:600,cursor:exporting?"not-allowed":"pointer",transition:"all .2s",fontFamily:"inherit"}}>
          {exporting ? <><span style={{animation:"spin 1s linear infinite",display:"inline-block"}}>⟳</span> Export en cours…</> : <>📥 Exporter en Excel</>}
        </button>
      </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))",gap:12,marginBottom:20}}>
        {[
          {label:"Total tickets",  val:total,       color:"#1a1a1a", icon:"🎫"},
          ...
          {label:"En révision",    val:inReview,    color:"#00BFFF", icon:"🔍"},
          ...
          <div key={i} style={card()}>
            ...
            <div style={{...value,color:kpi.color}}>{kpi.val}</div>
          </div>
        ))}
      </div>

      {/* ── Graphique statuts (barres) */}
          ...
          {statusStats.map(({col,count,cfg})=>{
            ...
            return(
              <div key={col} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:6}}>
                ...
                <div style={{fontSize:10,color:"#ADB5BD",fontWeight:600,textAlign:"center",letterSpacing:".3px"}}>{col}</div>
              </div>
            );
          })}
        </div>
        {total===0&&<div style={{textAlign:"center",color:"#ADB5BD",fontSize:13,padding:"20px 0"}}>Aucun ticket dans la période sélectionnée</div>}
      </div>
      ...
      <div style={card()}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16}}>
          <div style={{fontWeight:700,fontSize:14,color:"#1a1a1a"}}>Liste détaillée des tickets</div>
            ...
            :filtered.map(t=>{
              ...
              return(
                <div key={t.id} style={{display:"grid",gridTemplateColumns:"60px 2fr 1fr 1fr 1fr 1fr",gap:12,padding:"10px 12px",borderBottom:"1px solid #F8F9FA",alignItems:"center",transition:"background .15s"}}
                  ...
                  <div style={{fontWeight:500,fontSize:13,color:"#1a1a1a",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{t.title}</div>
                  <Badge color={COL_CFG[t.status]?.dot} bg={COL_CFG[t.status]?.bg}>{t.status}</Badge>
                  <Badge color={prio.color} bg={prio.bg}>{prio.icon} {prio.label}</Badge>
                  <div style={{display:"flex",alignItems:"center",gap:6}}>
                    {member?<><span style={{fontSize:13}}>{member.avatar}</span><span style={{fontSize:12,color:"#495057"}}>{member.name}</span></>:<span style={{fontSize:12,color:"#ADB5BD"}}>—</span>}
                  </div>
          ...
          {["chat","settings","adminpanel"].includes(activePage)&&(
            <div style={{textAlign:"center",padding:"80px 0",animation:"fadeIn .3s ease"}}>
              <div style={{fontSize:48,marginBottom:16}}>
                {activePage==="chat"?"💬":activePage==="settings"?"⚙️":"👤"}
              </div>
              <div style={{fontWeight:700,fontSize:20,color:"#1a1a1a",marginBottom:8}}>
                {activePage==="chat"?"Chat d'équipe":activePage==="settings"?"Paramètres":"Administration"}
              </div>
